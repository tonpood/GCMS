...
if (INSTALL_INIT == 'install') {
    ...
    if (isset($_POST['newdb'])) {
        ...
        if (DB_DRIVER == 'mysql') {
            // mysql database
            $db = sql("mysql://$config[db_username]:$config[db_password]@$config[db_server]");
        } else if (DB_DRIVER == 'mysqli') {
            // mysqli database
            $db = sql("mysqli://$config[db_username]:$config[db_password]@$config[db_server]");
        } else {
            // PDO mysql database
            $db = sql("pdo://$config[db_username]:$config[db_password]@$config[db_server]?dbdriver=mysql");
        }
