...
if (MAIN_INIT == 'admin' && $isMember) {
	...
	if (!$index) {
		...
		foreach ($config['languages'] AS $language) {
			...
			$content[] = '<span class="g-input icon-edit"><input type=text id=write_topic_'.$language.' name=write_topic_'.$language.' value="'.$item['topic'].'" maxlength=255 title="{LNG_TOPIC_COMMENT}"></span>';
			...
			$content[] = '<span class="g-input icon-tags"><input type=text id=write_keywords_'.$language.' name=write_keywords_'.$language.' value="'.$item['keywords'].'" maxlength=255 title="{LNG_TAGS_COMMENT}"></span>';
			...
			$content[] = '<span class="g-input icon-document"><input type=text id=write_relate_'.$language.' name=write_relate_'.$language.' value="'.$item['relate'].'" maxlength=255 title="{LNG_RELATE_COMMENT}"></span>';
			...
			$content[] = '<span class="g-input icon-file"><textarea id=write_description_'.$language.' name=write_description_'.$language.' rows=3 maxlength=255 title="{LNG_DESCRIPTION_COMMENT}">'.gcms::detail2TXT($item, 'description').'</textarea></span>';
