...
if (MAIN_INIT == 'admin' && gcms::canConfig($config, 'event_can_write')) {
	...
	if (sizeof($index) == 1) {
		...
		$content[] = '<span class="g-input icon-tags"><textarea name=write_keywords id=write_keywords rows=3 maxlength=255 title="{LNG_TAGS_COMMENT}">'.gcms::detail2TXT($index, 'keywords').'</textarea></span>';
